...
const recommendedItems = [
    ...
    // --- 衛生用品 ---
    { id: 'item_6', category: 'hygiene', name: '非常用トイレ', recommendedQuantity: '15', unit: '回分', description: '断水時の必需品', importance: '断水するとトイレは流せません。水が使えたとしても、集合住宅で配管が破損していた場合、汚水が逆流し下階のトイレからあふれる可能性もあります。トイレを我慢するために、食事や水を飲む量を減らすと、健康状態の悪化にもつながるので、家庭の便器に設置して固めて処理できる携帯トイレ・簡易トイレを用意しましょう。', simulated: true },
    ...
    { id: 'item_20', category: 'hygiene', name: '口内洗浄液', recommendedQuantity: '1', unit: '本', description: '1本 (9回分)。口腔ケアで感染症予防', importance: '水道が使えず歯磨きができない場合に備えて、口内洗浄液を準備しておきましょう。お口の健康は体の健康にも影響します。水不足で口の中を清潔に保つことができないと、口の中の菌が体に悪影響を及ぼす可能性があります。特に肺炎になりやすい高齢者の方は注意が必要です。', simulated: true },
                      ...
                      if (details && selected) {
                        if (details.resourceType === 'water') {
                          ...
                          userQuantity = (details.resourceAmount || 0) * selected.quantity / 1000;
                            ...
                            if (itemId === 20) { // 口内洗浄液
                                userQuantity = selected.quantity; // 「個数」でカウント
                            
                            // ▼▼▼ 【追加】 ID: 5 (救急セット) の計算ロジックを変更 ▼▼▼
                            } else if (itemId === 5) { // 救急セット
                                userQuantity = selected.quantity; // 「個数」でカウント
                            // ▲▲▲ 追加ここまで ▲▲▲
